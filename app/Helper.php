...
class Helper
{
	...
	public static function formatDate($date, $time = false)
	{
		...
		} elseif ($time == false || !is_numeric($date)) {
	...
	public static function createImageSlug($string, $ignoreId = null)
	{
		$slug = \Illuminate\Support\Str::slug($string, '-');
		if (empty($slug)) {
			$slug = 'prompt-' . time() . '-' . rand(100, 999);
		}
		$originalSlug = $slug;
		$count = 1;
		while (\App\Models\Images::where('slug', $slug)->where('id', '!=', $ignoreId)->exists()) {
