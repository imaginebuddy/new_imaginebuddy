...
trait SearchTrait {
    ...
    /**
     * Scope a query that matches a full-text search of term across title and tags with relevance scoring.
     *
    ...
    public function scopeSearch($query, $term)
    {
        ...
        return $query->whereStatus('active')
            ->where(function($q) use ($booleanQuery, $expandedTerms) {
                $q->whereRaw("MATCH(title, tags) AGAINST(? IN BOOLEAN MODE)", [$booleanQuery]);

                foreach ($expandedTerms as $synonym) {
                    $synonymBool = SearchQueryEngine::buildFulltextBooleanQuery($synonym);
                    if (!empty($synonymBool)) {
                        $q->orWhereRaw("MATCH(title, tags) AGAINST(? IN BOOLEAN MODE)", [$synonymBool]);
                    }
                }
            })
            ->groupBy('id')
            ->orderByRaw("MATCH(title, tags) AGAINST(? IN BOOLEAN MODE) DESC", [$booleanQuery]);
    }

    /**
     * Scope a query that matches a LIKE search of term across title and tags with AND matching and synonym expansion.
     *
     * @param \Illuminate\Database\Eloquent\Builder $query
     * @param string $term
     * @return \Illuminate\Database\Eloquent\Builder
