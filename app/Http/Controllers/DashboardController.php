...
class DashboardController extends Controller
{
  ...
  public function photos()
  {
    ...
    $data = auth()->user()->allImages()->orderBy('id', 'desc')->countLikesDownloads()->paginate($pagination);
    ...
    if (isset($sort) && $sort == 'title') {
      $data = Images::whereUserId(auth()->id())->orderBy('title', 'asc')->countLikesDownloads()->paginate($pagination);
    }

    if (isset($sort) && $sort == 'pending') {
      $data = Images::whereUserId(auth()->id())->where('status', 'pending')->countLikesDownloads()->paginate($pagination);
    }
  ...
  {
    $data = Purchases::with(['images', 'author'])->leftJoin('images', function ($join) {
  ...
  {
    $data = Purchases::with(['images', 'invoice'])->whereUserId(auth()->id())
      ->orderBy('id', 'DESC')
      ->whereApproved('1')
      ->whereMode('normal')
      ->paginate(20);
