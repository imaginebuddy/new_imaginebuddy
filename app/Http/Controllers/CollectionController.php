...
class CollectionController extends Controller {
   ...
   */
	 public function store( Request $request ) {
		...
		$sql->type    = $isAdmin ? ($request->type ? 'private' : 'public') : 'private';
		$sql->user_id = auth()->id();
		$sql->save();
		...
		return response()->json([
			        'success' => true,
			        'data' => '<div class="form-check mb-1">
							<input class="no-show form-check-input addImageCollection addListUser" data-image-id="'.$request->image_id.'" data-collection-id="'.$idCollection.'" name="checked" type="checkbox" value="true" id="collectionCheck'.$idCollection.'">
							<label class="form-check-label text-overflow" for="collectionCheck'.$idCollection.'">
							'.($sql->type == 'private' ? '<i class="bi bi-lock-fill me-1 text-muted"></i> ' : '').trim(e($request->title)).'
							</label>
						</div>',
			    ]);
    ...
    public function addImageCollection(Request $request){
	    ...
	    // Verify user
		if( isset( $collectionsImage ) && $collectionsImage->belongsCollection->user_id != auth()->id() ) {
			return response()->json([
				        'status' => false,
	...
	// Edit Collection
	 public function edit( Request $request ) {
		 ...
		 if (! isset($sql) || $sql->user_id != auth()->id() ) {
		 	return response()->json([
			        'not_authorized' => true,
			    ]);
		 }
