...
(function ($) {
	...
	$(document).ready(function () {
		...
		/*= Like =*/
		$(document).off('click', '.likeButton').on('click', '.likeButton', function (e) {
			var element = $(this);
			var id = element.attr("data-id");
			var like = element.attr('data-like') || 'Like';
			var like_active = element.attr('data-unlike') || 'Liked';
			var data = 'id=' + id;
			...
			if (element.hasClass('active') || element.hasClass('btn-liked')) {
				element.removeClass('active btn-liked');
				element.find('i').removeClass('bi-heart-fill text-danger').addClass('bi-heart');
				element.find('.textLike').html(like).removeClass('text-danger');
			} else {
				element.addClass('active btn-liked');
				element.find('i').removeClass('bi-heart').addClass('bi-heart-fill text-danger');
				element.find('.textLike').html(like_active).addClass('text-danger');
			}
	...
	/*= Add collection  =*/
	$(document).on('click', '#addCollection, #btnCollection', function (e) {
		var element = $(this);
		var form = $('#addCollectionForm, #formAddCollection');
		...
		$.ajax({
			...
			success: function (result) {
				...
				if (result.success) {
					...
					$(result.data).hide().appendTo('.collectionsData').slideDown(200);

					$('.no-collections').remove();
					$("#titleCollection, input[name='title']").val('');
					$('#dangerAlertCollection, #dangerAlert').fadeOut();
					...
					var $key = '';
					for ($key in result.errors) {
						error += '<li><i class="bi bi-x-circle me-1"></i> ' + result.errors[$key] + '</li>';
					}

					$('#showErrorsCollection, #showErrors').html(error);
					$('#dangerAlertCollection, #dangerAlert').fadeIn(500);
